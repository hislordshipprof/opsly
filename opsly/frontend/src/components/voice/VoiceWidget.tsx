...
export default function VoiceWidget() {
  ...
  return (
    <div className="flex w-full max-w-lg flex-col rounded-lg border border-border bg-card">
      ...
      {/* Controls — two rows: action buttons + text input */}
      <div className="border-t border-border p-4 space-y-3">
        {/* Row 1: Photo + Visualizer + Mic */}
        <div className="flex items-center justify-center gap-4">
          ...
          <button
            ...
            className="size-10 rounded-full bg-muted/60 hover:bg-muted flex items-center justify-center transition-colors disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
            aria-label="Upload photo"
            title={currentWorkOrderId ? 'Upload photo' : 'Create a work order first'}
          >
            {isUploadingPhoto ? (
              <svg className="size-5 animate-spin text-foreground" fill="none" viewBox="0 0 24 24">
        ...
        <p className="text-center text-xs text-muted-foreground">
          {state === 'IDLE'
            ? 'Tap mic for voice, or type below'
